...
function showFirstAndLast(arr){
    ...
    arr.forEach(function(val) {
       ...
       firstLastChar.push(firstChar + lastChar);
    });
...
function addKeyAndValue(arr,key,value){
    ...
    arr.forEach(function(val) {
        val[key] = value;
        additionalKeyValue.push(val);
    });
    return additionalKeyValue;
};
...
function valTimesIndex(arr){
    let multiply = []
    arr.map(function(val) {
        const indexLocation = arr.indexOf(val);
        const multipleOfVal = val * indexLocation;
        multiply.push(multipleOfVal);
    });
    return multiply;
};
